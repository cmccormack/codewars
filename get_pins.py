...
(get_pins in python). But please note that all PINs, the observed one and
also the results, must be strings, because of potentially leading '0's.
...
def get_pins(observed):
    ...
    #print keypad  # DELETE
    lists = [map(str, keypad[int(digit)]) for digit in observed]
    #print 'lists: ' + str(lists)
    return [''.join(keys) for keys in product(*lists)]
